...
//	StartOrStop(csLSS,	startLSS(d),	stopLSS(d))
	if(newCommunicationState->csLSS && d->CurrentCommunicationState.csLSS == 0){\
		MSG_WAR(0x9999,"startLSS(d)", 9999);\
		d->CurrentCommunicationState.csLSS = 1;\
		startLSS(d);\
	}else if(!newCommunicationState->csLSS && d->CurrentCommunicationState.csLSS == 1){\
		MSG_WAR(0x9999,"stopLSS(d)", 9999);\
		d->CurrentCommunicationState.csLSS = 0;\
		stopLSS(d);\
	}
#endif
//	StartOrStop(csSDO,	None,		resetSDO(d))
	if(newCommunicationState->csSDO && d->CurrentCommunicationState.csSDO == 0){\
		MSG_WAR(0x9999,"None", 9999);\
		d->CurrentCommunicationState.csSDO = 1;\
		None;\
	}else if(!newCommunicationState->csSDO && d->CurrentCommunicationState.csSDO == 1){\
		MSG_WAR(0x9999,"resetSDO(d)", 9999);\
		d->CurrentCommunicationState.csSDO = 0;\
		resetSDO(d);\
	}
//	StartOrStop(csSYNC,	startSYNC(d),		stopSYNC(d))
	if(newCommunicationState->csSYNC && d->CurrentCommunicationState.csSYNC == 0){\
		MSG_WAR(0x9999,"startSYNC(d)", 9999);\
		d->CurrentCommunicationState.csSYNC = 1;\
		startSYNC(d);\
	}else if(!newCommunicationState->csSYNC && d->CurrentCommunicationState.csSYNC == 1){\
		MSG_WAR(0x9999,"stopSYNC(d)", 9999);\
		d->CurrentCommunicationState.csSYNC = 0;\
		stopSYNC(d);\
	}
//	StartOrStop(csHeartbeat,	heartbeatInit(d),	heartbeatStop(d))
	if(newCommunicationState->csHeartbeat && d->CurrentCommunicationState.csHeartbeat == 0){\
		MSG_WAR(0x9999,"heartbeatInit(d)", 9999);\
		d->CurrentCommunicationState.csHeartbeat = 1;\
		heartbeatInit(d);\
	}else if(!newCommunicationState->csHeartbeat && d->CurrentCommunicationState.csHeartbeat == 1){\
		MSG_WAR(0x9999,"heartbeatStop(d)", 9999);\
		d->CurrentCommunicationState.csHeartbeat = 0;\
		heartbeatStop(d);\
	}
//	StartOrStop(csEmergency,	emergencyInit(d),	emergencyStop(d)) 
	if(newCommunicationState->csEmergency && d->CurrentCommunicationState.csEmergency == 0){\
		MSG_WAR(0x9999,"emergencyInit(d)", 9999);\
		d->CurrentCommunicationState.csEmergency = 1;\
		emergencyInit(d);\
	}else if(!newCommunicationState->csEmergency && d->CurrentCommunicationState.csEmergency == 1){\
		MSG_WAR(0x9999,"emergencyStop(d)", 9999);\
		d->CurrentCommunicationState.csEmergency = 0;\
		emergencyStop(d);\
	}
//	StartOrStop(csPDO,	"PDOInit(d)",	PDOStop(d))
	if(newCommunicationState->csPDO && d->CurrentCommunicationState.csPDO == 0){\
		MSG_WAR(0x9999,"PDOInit(d)", 9999);\
		d->CurrentCommunicationState.csPDO = 1;\
		PDOInit(d);\
	}else if(!newCommunicationState->csPDO && d->CurrentCommunicationState.csPDO == 1){\
		MSG_WAR(0x9999,"PDOStop(d)", 9999);\
		d->CurrentCommunicationState.csPDO = 0;\
		PDOStop(d);\
	}
//	StartOrStop(csBoot_Up,	None,	slaveSendBootUp(d))
	if(newCommunicationState->csBoot_Up && d->CurrentCommunicationState.csBoot_Up == 0){\
		MSG_WAR(0x9999,"None", 9999);\
		d->CurrentCommunicationState.csBoot_Up = 1;\
		None;\
	}else if(!newCommunicationState->csBoot_Up && d->CurrentCommunicationState.csBoot_Up == 1){\
		MSG_WAR(0x9999,"slaveSendBootUp(d)", 9999);\
		d->CurrentCommunicationState.csBoot_Up = 0;\
		slaveSendBootUp(d);\
	}
}
